    ...
    header.appendChild(collapseBtn);
     ...
     viewBtn.addEventListener('click', (e) => {
         ...
         if (catData && window.AppDetailsOverlay?.showDetailsOverlay) {
             // Ensure price history exists and is an array when showing details
             const categoryDataWithHistory = {
                  ...catData,
                  books: (catData.books || []).map(b => ({
                       ...b,
                       priceHistory: Array.isArray(b.priceHistory) ? b.priceHistory : []
                  }))
             };
             window.AppDetailsOverlay.showDetailsOverlay({ type: 'category', ...categoryDataWithHistory });
         } else {
...
function addSingleTrackerNodeElement(containerElement, book, categoryId, categoryColor, index) {
    ...
    try {
        const nodeData = {
            ...
            voucher_code: book.voucher_code,
            local_image_filename: book.local_image_filename,
            // Ensure price history is an array when storing
            priceHistory: Array.isArray(book.priceHistory) ? book.priceHistory : []
        };
        ...
        if (!e.target.closest('.remove-node-btn')) {
            try {
                ...
                if (window.AppDetailsOverlay?.showDetailsOverlay) {
                     // Ensure price history is an array before showing details
...
async function performPriceCheckCycle() {
    ...
    let itemsChecked = 0;
    let updatesFound = false; // Flag specifically for price *value* changes
    let historyUpdated = false; // Flag for any successful history addition
            ...
            for (const book of category.books) {
                if (book && book.link) {
                    ...
                    if (fetchedPrices && !fetchedPrices.fetchError) {
                        // --- Compare fetched prices (camelCase) with model prices (snake_case) ---
                        ...
                        if (priceChanged) {
                            ...
                            updatesFound = true; // Mark that a price value changed
                            // Update book object model (snake_case) with fetched values (camelCase)
                        ...
                        // --- Add to price history (using camelCase keys from fetched data) ---
    ...
    console.log(`[Tracker] Price check cycle finished in ${duration / 1000}s. Checked: ${itemsChecked}. Price Updates: ${updatesFound}. History Adds: ${historyUpdated}. Errors: ${errorsEncountered}.`);

    // Save data if any successful checks occurred (meaning history was updated)
    if (historyUpdated) { // Save if any history was added (implies successful fetch)
        if (updatesFound) {
            console.log("[Tracker] Price updates found, saving data...");
        } else {
             console.log("[Tracker] No price changes, but saving updated price history...");
        }
        await saveTrackerData('update prices/history');
        // Re-render tracker UI to show new prices *and* allow chart updates
        renderCategoriesAndBooks();
    } else if (itemsChecked > 0 && errorsEncountered === itemsChecked) { // All checks failed
         if (window.statusBar) window.statusBar.textContent = `Price check complete (${itemsChecked} items, all failed).`;
    } else { // No items to check, or some other case
         if (window.statusBar) window.statusBar.textContent = `Price check complete (${itemsChecked} items checked).`;
    }
